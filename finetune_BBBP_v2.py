...
# tox21_A1_ourmethod=tox21_A1
# tox21_A1_ourmethod=tox21_A1_ourmethod

# print(tox21_A1_ourmethod.shape)
...
# Tox21_ourimpute_train, Tox21_ourimpute_test, Tox21_ourimpute_train_y, Tox21_ourimpute_test_y = train_test_split(tox21_A1_ourmethod, y_nonan, test_size = 0.30, random_state=42,stratify=y_nonan)
class Tox21_Dataset(Dataset):
...
# resample_noimpute_train, resample_noimpute_train_y = rus.fit_resample(Tox21_ourimpute_train, Tox21_ourimpute_train_y)
inhibitor = [0, 1]



enc = preprocessing.OneHotEncoder(categories=[inhibitor])
enc.fit(Tox21_ourimpute_train_y.reshape(-1, 1))
Tox21_ourimpute_train_y_onehot=enc.transform(Tox21_ourimpute_train_y.reshape(-1, 1)).toarray()
Tox21_ourimpute_train_y_onehot=torch.tensor(Tox21_ourimpute_train_y_onehot , dtype=torch.float32)
...
enc_val.fit(Tox21_ourimpute_valid_y.reshape(-1, 1))
Tox21_ourimpute_valid_y_onehot=enc.transform(Tox21_ourimpute_valid_y.reshape(-1, 1)).toarray()
Tox21_ourimpute_valid_y_onehot=torch.tensor(Tox21_ourimpute_valid_y_onehot , dtype=torch.float32)




# rus_val = RandomUnderSampler(random_state=0)
# resample_noimpute_test, resample_noimpute_test_y = rus_val.fit_resample(Tox21_ourimpute_test, Tox21_ourimpute_test_y)
enc_test = preprocessing.OneHotEncoder(categories=[inhibitor])
enc_test.fit(Tox21_ourimpute_test_y.reshape(-1, 1))
Tox21_ourimpute_test_y_onehot=enc.transform(Tox21_ourimpute_test_y.reshape(-1, 1)).toarray()
Tox21_ourimpute_test_y_onehot=torch.tensor(Tox21_ourimpute_test_y_onehot , dtype=torch.float32)



Tox21_imputed_ourmethod_ds = Tox21_Dataset(X=Tox21_ourimpute_train,y=Tox21_ourimpute_train_y_onehot)
Tox21_imputed_ourmethod_dl = DataLoader(Tox21_imputed_ourmethod_ds, batch_size=128, shuffle=True, num_workers=1)
...
Tox21_valid_ds = Tox21_Dataset(X=Tox21_ourimpute_valid,y=Tox21_ourimpute_valid_y_onehot)
sampler = torch.utils.data.RandomSampler(Tox21_valid_ds, replacement=False)
Tox21_valid_dl = DataLoader(Tox21_valid_ds, batch_size=128, shuffle=False, num_workers=1, sampler=sampler)

Tox21_test_ds = Tox21_Dataset(X=Tox21_ourimpute_test,y=Tox21_ourimpute_test_y_onehot)
sampler_test = torch.utils.data.RandomSampler(Tox21_test_ds, replacement=False)
Tox21_test_dl = DataLoader(Tox21_test_ds, batch_size=128, shuffle=False, num_workers=1, sampler=sampler_test)


def train_Tox21(model,train_loader):
    model.train()
    losses=[]
...
def valid_Tox21(model, valid_dl):
    df = []
    model.eval()
...
#     print(HIV_A1_ourmethod_all)
    ...
    df_tmp ={"Method":"VAE_finetune",
                         "ROC-AUC":ROC_AUC, "Dataset":"Tox21", "Input":"Impute"}
    df.append(df_tmp)
    return df
...
for epoch in range(1, 10):
    ...
    train_Tox21(model=model, train_loader=Tox21_imputed_ourmethod_dl)
    Tox21_our_impute=valid_Tox21(model=model, valid_dl=Tox21_valid_dl)
Test_result=Test_one_epoch(model=model, test_dl=Tox21_test_dl)
print("End our method Finetune")

###### load SNN impute Tox21

# print("Start SNN")
# SNN_impute = pd.read_csv("SNN_imputed_Tox21.csv")
# # SNN_impute=pd.DataFrame(SNN_impute)
# # SNN_impute.shape
# # ~tox21_data["A1.0"].isna()
# # SNN_impute=SNN_impute.iloc[~tox21_data["A1.0"].isna(),:]
# # # SNN_impute.head()
# # SNN_impute=SNN_impute.iloc[]
# # sample_should_filter=(~np.isnan(y)|~tox21_data["A1.0"].isna())

# SNN_All_=SNN_impute.iloc[:,1:]
# SNN_All_.shape
# # # Tox21_A2_data=Tox21_A2.signature[~np.isnan(y),]
# # # Tox21_A3_data=Tox21_A3.signature[~np.isnan(y),]
# # # Tox21_B1_data=Tox21_B1.signature[~np.isnan(y),]
# # # Tox21_B4_data=Tox21_B4.signature[~np.isnan(y),]



# # # ##### compare with no imputation
# # # # SNN_All=np.concatenate([tox21_A1, Tox21_A2_data, Tox21_A3_data,Tox21_B1_data ,Tox21_B4_data], axis=1)
# X_SNN=SNN_All_
