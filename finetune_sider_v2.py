...
BACE_data=pd.read_csv("extend_dataset/Sider/Sider_train_3over4.csv")

# BACE_data=BACE_data[~BACE_data["A1.0"].isna()]
BACE_ourimpute_train=BACE_data.iloc[:,0:3200].fillna(0.0).values
# BACE_ourimpute_train_y=BACE_data.loc[:,"CT_TOX"].values
BACE_ourimpute_train_y_tmp=BACE_data.iloc[:,3200].values
BACE_ourimpute_train_y=BACE_data.iloc[:,3200:-1].values



BACE_data_valid=pd.read_csv("extend_dataset/Sider/Sider_valid_3over4.csv")

# BACE_data=BACE_data[~BACE_data["A1.0"].isna()]
BACE_ourimpute_valid=BACE_data_valid.iloc[:,0:3200].fillna(0.0).values
# BACE_ourimpute_valid_y=BACE_data_valid.loc[:,"CT_TOX"].values
BACE_ourimpute_valid_y_tmp=BACE_data_valid.iloc[:,3200].values
BACE_ourimpute_valid_y=BACE_data_valid.iloc[:,3200:-1].values


BACE_data_test=pd.read_csv("extend_dataset/Sider/Sider_test.csv")

# BACE_data=BACE_data[~BACE_data["A1.0"].isna()]
BACE_ourimpute_test=BACE_data_test.iloc[:,0:3200].fillna(0.0).values
# BACE_ourimpute_test_y=BACE_data_test.loc[:,"CT_TOX"].values
BACE_ourimpute_test_y_tmp=BACE_data_test.iloc[:,3200].values
BACE_ourimpute_test_y=BACE_data_test.iloc[:,3200:-1].values
...
model.fine_tune.fc2 = nn.Linear(128, 128) # 定义一个新的FC层

model.fine_tune.fc3 = nn.Linear(128, 128) # 定义一个新的FC层

model.fine_tune.fc4 = nn.Linear(128, 128) # 定义一个新的FC层

model.fine_tune.fc5 = nn.Linear(128, 128) # 定义一个新的FC层

# model.fine_tune.rl1 = nn.ReLU()
model.fine_tune.fc6 = nn.Linear(128, 64) # 定义一个新的FC层
# model.fine_tune.rl1 = nn.ReLU()
model.fine_tune.fc7 = nn.Linear(64, 27) # 定义一个新的FC层
# model.fine_tune.rl2 = nn.ReLU()
# model.fine_tune.fc8 = nn.Linear(32, 27) # 定义一个新的FC层
...
BACE_imputed_ourmethod_ds = BACE_Dataset(X=BACE_ourimpute_train,y=BACE_ourimpute_train_y)
BACE_imputed_ourmethod_dl = DataLoader(BACE_imputed_ourmethod_ds, batch_size=64, shuffle=True, num_workers=1)
...
BACE_valid_ds = BACE_Dataset(X=BACE_ourimpute_valid,y=BACE_ourimpute_valid_y)
sampler = torch.utils.data.RandomSampler(BACE_valid_ds, replacement=False)
BACE_valid_dl = DataLoader(BACE_valid_ds, batch_size=64, shuffle=False, num_workers=1, sampler=sampler)

BACE_test_ds = BACE_Dataset(X=BACE_ourimpute_test,y=BACE_ourimpute_test_y)
sampler_test = torch.utils.data.RandomSampler(BACE_test_ds, replacement=False)
BACE_test_dl = DataLoader(BACE_test_ds, batch_size=64, shuffle=False, num_workers=1, sampler=sampler_test)
