...
tox21_data=pd.read_csv("Tox21_extend.csv")

tox21_data=tox21_data[~tox21_data["A1.0"].isna()]
# tox21_data.iloc[:,E4_start:E4_end]=0
tox21_A1=tox21_data.iloc[:,2:3202]

y=tox21_data.iloc[:,3202]
# y
y_nonan=y[~np.isnan(y)]
y_nonan=y_nonan.values
tox21_A1_ourmethod=tox21_A1[~np.isnan(y)].fillna(0.0)
tox21_A1_ourmethod=tox21_A1_ourmethod.values
...
# enc.fit(Tox21_ourimpute_train_y.values.reshape(-1,1))
# # print(resample_noimpute_train_y)
# Tox21_ourimpute_train_y_onehot=enc.transform(Tox21_ourimpute_train_y.values.reshape(-1,1)).toarray()
...
enc_val.fit(resample_noimpute_test_y.reshape(-1,1))
Tox21_ourimpute_test_y_onehot=enc.transform(resample_noimpute_test_y.reshape(-1,1)).toarray()
Tox21_ourimpute_test_y_onehot=torch.tensor(Tox21_ourimpute_test_y_onehot , dtype=torch.float32)



Tox21_imputed_ourmethod_ds = Tox21_Dataset(X=resample_noimpute_train,y=resample_noimpute_train_y_onehot)
...
# Tox21_imputed_ourmethod_ds = Tox21_Dataset(X=resample_noimpute_train,y=resample_noimpute_train_y_onehot)
# Tox21_imputed_ourmethod_dl = DataLoader(Tox21_imputed_ourmethod_ds, batch_size=1, shuffle=True, num_workers=1)


Tox21_valid_ds = Tox21_Dataset(X=resample_noimpute_test,y=Tox21_ourimpute_test_y_onehot)
