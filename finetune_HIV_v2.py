...
BACE_data=pd.read_csv("extend_dataset/HIV/HIV_train_1over4.csv")

# BACE_data=BACE_data[~BACE_data["A1.0"].isna()]
BACE_ourimpute_train=BACE_data.iloc[:,2:3202].fillna(0.0).values
BACE_ourimpute_train_y=BACE_data.loc[:,"HIV_active"].values


BACE_data_valid=pd.read_csv("extend_dataset/HIV/HIV_valid_1over4.csv")

# BACE_data=BACE_data[~BACE_data["A1.0"].isna()]
BACE_ourimpute_valid=BACE_data_valid.iloc[:,2:3202].fillna(0.0).values
BACE_ourimpute_valid_y=BACE_data_valid.loc[:,"HIV_active"].values



BACE_data_test=pd.read_csv("extend_dataset/HIV/HIV_test.csv")

# BACE_data=BACE_data[~BACE_data["A1.0"].isna()]
BACE_ourimpute_test=BACE_data_test.iloc[:,2:3202].fillna(0.0).values
BACE_ourimpute_test_y=BACE_data_test.loc[:,"HIV_active"].values
...
BACE_ourimpute_train_y_onehot=enc.transform(resample_noimpute_train_y.reshape(-1, 1)).toarray()
BACE_ourimpute_train_y_onehot=torch.tensor(BACE_ourimpute_train_y_onehot , dtype=torch.float32)
...
from imblearn.under_sampling import RandomUnderSampler
rus_valid = RandomUnderSampler(random_state=0)
resample_noimpute_valid, resample_noimpute_valid_y = rus_valid.fit_resample(BACE_ourimpute_valid, BACE_ourimpute_valid_y)

enc_val = preprocessing.OneHotEncoder(categories=[inhibitor])
enc_val.fit(resample_noimpute_valid_y.reshape(-1, 1))
BACE_ourimpute_valid_y_onehot=enc.transform(resample_noimpute_valid_y.reshape(-1, 1)).toarray()
...
resample_noimpute_test, resample_noimpute_test_y = rus_test.fit_resample(BACE_ourimpute_test, BACE_ourimpute_test_y)
...
enc_test.fit(resample_noimpute_test_y.reshape(-1, 1))
BACE_ourimpute_test_y_onehot=enc.transform(resample_noimpute_test_y.reshape(-1, 1)).toarray()
BACE_ourimpute_test_y_onehot=torch.tensor(BACE_ourimpute_test_y_onehot , dtype=torch.float32)

BACE_imputed_ourmethod_ds = BACE_Dataset(X=resample_noimpute_train,y=BACE_ourimpute_train_y_onehot)
BACE_imputed_ourmethod_dl = DataLoader(BACE_imputed_ourmethod_ds, batch_size=128, shuffle=True, num_workers=1)


BACE_valid_ds = BACE_Dataset(X=resample_noimpute_valid,y=BACE_ourimpute_valid_y_onehot)
...
BACE_test_ds = BACE_Dataset(X=resample_noimpute_test,y=BACE_ourimpute_test_y_onehot)
